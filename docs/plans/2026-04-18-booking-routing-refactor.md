...
- [x] Удалить `selectedEventTypeIdAtom` если больше нигде не используется
...
- [x] Убедиться, что все маршруты зарегистрированы в `routes.ts` и реэкспортированы
- [x] Убедиться, что `navigate.*` helpers покрывают все переходы флоу
- [x] Запустить полный `pnpm test` — все тесты зелёные (179/179)
- [x] Запустить `pnpm type-check` — без ошибок (web: 0 новых ошибок; API: pre-existing Prisma errors)
- [x] Запустить `pnpm build` — web сборка успешна (API: pre-existing Prisma errors)
- [x] Сделать итоговый коммит
